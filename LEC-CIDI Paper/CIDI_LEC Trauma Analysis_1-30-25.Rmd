...
The latest models and figures start at: **Regression update 1-13-25**
...
We are using the latest NeuroMex Datafreeze 6
```{r message=FALSE, warning=FALSE}
setwd("~/Dropbox (Harvard University)/NeuroMex/6. Data Freezes/Data Freeze 6") 
nmex <- read.csv("NeuroMex_Freeze6_Draft1.csv")
```
...
We remove participants who did not complete UBACC or left the interview prematurely. 
```{r message=FALSE, warning=FALSE}

#These participants are missing the UBACC
nmex <- nmex %>% filter(!(partid %in% 
                            c("CAP5649381", "CAP9216055", 
                              "GEA7332459", "QUP2253899", "QUP5901822", 
                              "QUP6859222")))
...
nmex <- nmex %>% filter(!(partid %in% 
                            c("INP2366191", "INP4404950", 
                              "JAP1320792", "INP1389432")))
...
Here, we code for interpersonal trauma, noninterpersonal trauma. We take the add
of each trauma (binary) for "happened to me only", which will give a sum of traumas experienced. 
...
reg_data <- df_trauma_only %>% filter(model == "Model D") %>%
